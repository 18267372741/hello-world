<template lang="html">
	...
	<div class="row box">
		<div class="col-sm-8 row picBox">
            <template v-if="loadBigZheZhao==true">
                ...
                <div class="col-sm-12 pic tuBox">
                    ...
                    <div v-for="myPic in myPic.myPic">
                        <img v-if="!myPic.imgUrl" @click.stop="findAllEndPic($event,myPic,'add')" src="../assets/images/wenjianjia3.png" class="wenjian" />
                        <img data-toggle="modal" data-target=".tuya" @click="tuyas($event,myPic.imgUrl,myPic.NO,'add',$index)" v-on:error="yuantu($index,$event)" v-if="myPic.imgUrl" :src="qnyUrl+myPic.imgUrl+'?imageView2/1/w/250/h/250'" class="imgtu" />
                        <p v-if="!myPic.imgUrl" class="beijing">
                            <span class="name">{{myPic}}</span>
                        </p>
                        <p v-if="myPic.imgUrl" class="beijing">
                            <!-- <span v-if="!myPic.FG" class="NO">{{myPic.NO}}</span> -->
                            <span class="names">{{myPic.imgUrl.split("/")[mulvNum.length+1].split("_")[0]}}</span>
                        </p>
                    </div>
                </div>
            </template>
		...
		<!-- <div v-if="xsyc.wenjianjia" class="col-sm-8 row picBox">
			<div class="col-sm-12 pic wenjianBigBox">
				<div class="wenjianBox" v-for="myPic in myPic.wenjianjia">
					<img @click="findAllEndPic($event,myPic)" src="../assets/images/wenjianjia.png" class="wenjian" />
					<p class="beijing">
						<span class="name">{{myPic}}</span>
					</p>
				</div>
			...
			<form class="col-sm-12">
				<div class="row">
					<div class="row col-sm-12">
						...
						<div class="col-sm-3">
							<button v-if="addr.down" type="button" data-toggle="modal" data-target=".xiazai" class="btn btn-primary btn-xs">打包下载</button>
						</div>
						<!-- <div class="col-sm-3">
							<a href="#/index/orderMan">
								<button type="button" class="btn btn-default btn-xs">返回</button>
							</a>
						</div> -->
					</div>
				</div>
			...
			<table class="table table-condensedn table-hover">
				<tbody>
					<tr v-for="tuyaLists in tuyaListss">
						...
						<!-- <td style="text-align:left;padding-left:25px" v-if="tuyaLists.NO=='返工图'">{{tuyaLists.NO}}<br>{{tuyaLists.name}}</td> -->
		...
		<div class="modal fade xiazai" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					<div class="modal-body">
						<p>
							您已下载{{orderInfo.downloadNum}}次，免费共可下载3次，
							<span v-if="(3-orderInfo.downloadNum)>0">还可以免费下载{{3-addr.down}}次，</span>
							<span v-if="(3-orderInfo.downloadNum)<=0" style="color:red">下载将扣除您一元，</span>
							确认下载吗？
						</p>
					</div>
					<div class="modal-footer">
						<button @click="bale" type="button" class="btn btn-primary sure" data-dismiss="modal">确定</button>
    ...
    export default {
        ...
        methods: {
            ...
            findFGPic() { //查看返工图片
                ...
                var id = "";
                if (this.addr.per == "XT") {
                    id = this.addr.workxtid;
                } else if (this.addr.per == "SJ") {
                    id = this.addr.worksjid;
                }
                postParam = {
                    "pageSize": 1000
                };
                if (this.myPic.name) {
                    postParam.prefix = id + this.addr.id + "fgImg" + this.addr.per + this.addr.title + "/" + this.myPic.name;
                } else if (!this.myPic.name) {
                    postParam.prefix = id + this.addr.id + "fgImg" + this.addr.per + this.addr.title;
                }
            ...
            downLoadUrl() {
                var url = this.host + "v1/photo/order/downloadFile",
                    self = this,
                    postParam = {};
                postParam = {
                    "orderId": parseInt(this.addr.id),
            ...
            bale() { //打包下载
                this.$broadcast('parent-lock-page-on', "下载中");
                ...
                postParam = {
                    "imgUrl": imgUrls,
                    "keys": this.down.yuantu[0].imgUrl,
            ...
            findbaleSess(id) { //查询是否打包成功
                ...
                Lib.M.load(url, postParam, function(data) {
                    if (data.code == 0) {
                        ...
                        setTimeout(function() {
                            self.findbaleSess(id);
                            this.$broadcast('parent-lock-page-off', "下载中");
                            Lib.M.topRightInfoTips({
                                title: "下载失败"
                            });
                        }, 500)
...
<style scoped>
    ...
    .imgtu {
        box-shadow: 2px 3px 5px 0px rgba(9, 2, 4, 0.257);
    }
