<template lang="html">
    <div class="shangchuan" v-el:qi-niu-container>
        <div class="btn btn-default jianjiao1" v-el:qi-niu-button>
            <i class="iconfont icon-001" style="font-size:14px"></i>
            <span>上传图片</span>
        </div>
        <div @click="downUp" class="btn btn-primary jianjiao2">
        ...
        <div class="neirong">
            <span v-if="!upPic.list.length" class="tuozhuai">请将压缩包拖拽到此处</span>
            <table class="table table-condensedn table-hover">
                ...
                <tbody class="stop_load">
                    <tr v-for="list in upPic.list">
                        <td style="width:100px;overflow:hidden;text-align:left">{{list.names}}</td>
                        <td style="width:100px;text-align:left">{{(list.size/1000000).toFixed(2)}}M</td>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.key = "archive/" + this.addrQN.id;
                ...
                var defaultOption = {
                    ...
                    init: {
                        ...
                        'BeforeUpload': function(up, file) {
                            ...
                            // var geshi = file.name.split(".")[1]
                            // if( geshi!= "rar" &&geshi!="ios"&&geshi!="7z"&&geshi!="zip" ){
                            //     console.log(28)
                            //     return false;
                            // }
                            self.$dispatch('child-qiniu-data', file);
                        ...
                        'Error': function(up, err, errTip) {
                            ...
                            // if (err.code == -600) {
                            //     Lib.M.topRightInfoTips({
                            //         title: "请上传规定大小的图片(默认1M，有说明请按说明传规定大小的图片)"
                            //     });
                            // }
                            if (err.code == -200) {
                                Lib.M.topRightInfoTips({
                                    title: "您的网络可能信号不好，请重新上传"
                                });
...
<style type="text/css" scoped>
    ...
    .neirong {
        ...
        border-radius: 6px;
        background: white;
        /*background: url(../../assets/tuozhua.png) no-repeat center white;*/
    }
