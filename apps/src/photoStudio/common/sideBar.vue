    ...
    <div class="sideBar">
    	<ul>
    		...
    		<li>
    			<ul class="sideNav"> 
		    		<li>
		    			<a :class="{active: linkType=='sendOrder'}" v-link-active v-link="{path: '/index/sendOrder', activeClass: 'active'}">
		    				<i class="iconfont icon-fabudingdan"></i>
		    				<span class="text">我要发单</span>
		    			</a>
		    		</li>
		    		<li>
		    			<a :class="{active: linkType=='orderMan'}" style="position: relative;" v-link-active v-link="{path: '/index/orderMan', activeClass: 'active'}">
    ...
    export default {
		...
		created: function () {
			window.addEventListener('hashchange', () => {
				...
				if(/index\/sendOrder2/g.test(href)){
		...
		events: { },
		ready: function () {
			if(/index\/sendOrder2/g.test(location.href)){
				this.$set('linkType', 'sendOrder');
			}
        ...
        methods: {
        	toushus (){
				...
				postParam = {
					...
					"photoInfoId":parseInt(this.id)//工作室id
				};
