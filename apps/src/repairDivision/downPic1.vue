<template lang="html">
	...
	<div class="row box">
		<div class="col-sm-8 row picBox">
			...
			<strong style="position: absolute; top: 38px; left: 100px;z-index: 1;font-weight: normal;">{{mulvNumLook[mulvNumLook.length-1]}}</strong>
		...
		<div class="col-sm-3 listInfo">
			<form class="col-sm-12">
				<div class="row">
					<p v-if="card==addr.per&&addr.status=='REWORK'" style="margin-bottom: 10px;display: flex; justify-content: space-around; padding: 0 50px;">
						<button @click="xianshi" class="btn btn-default">
				            <i class="iconfont icon-001" style="font-size:14px"></i>
				            <span>上传图片</span>
				        </button>
                        <button v-if="card==addr.per&&addr.status=='REWORK'" type="button" class="btn btn-default" data-toggle="modal" data-target=".accept">提交审核</button>
					</p>
				</div>
			</form>
			...
			<div class="modal-dialog modal-lg" style="width:100%">
				<div style="height:100%;display:table;margin: 0 auto;margin-bottom:20px">
					<a :href="qnyUrl+tuya.url" target="_blank">
						<img v-on:load="loading" v-on:error="bad($event)" style="height:870px" :src="qnyUrl+tuya.url+houzuidaxiao" :class="{'touming':finish!='finish'}" class="tupian"/>
			...
			<div class="modal-dialog modal-lg picMutai" style="width:100%">
				<div style="height:100%;display:table;margin: 0 auto;">
					<a style="display:table-cell;vertical-align:middle;" :href="qnyUrl+tuya.url" target="_blank">
						<img v-on:error="bad($event)" v-on:load="loading" style="height:870px" :src="qnyUrl+tuya.url+houzuidaxiao" :class="{'touming':finish!='finish'}" class="tupian"/>
                        <div v-if="finish!='finish'" class="load1">
                            <div class="loader"></div>
                        </div>
					</a>
...
<script>
    ...
    export default {
        data: () => ({
            ...
            onOff: true, //开关
            houzuidaxiao: "?imageView2/0/w/2500", //后缀
            mulvNum: [], //目录次数
            mulvNumLook: [], //预览目录次数
            finish: "", //终止
        ...
        created: function() {
            ...
            this.fgImg.key = this.addr.orderid + "/" + "fgImg" + this.addr.per;
            this.findRework();
            this.findAllEndPic();
        },
        events: {
        ...
        methods: {
            ...
            audit() { //提交审核
                ...
                if (this.addr.per == "XT") {
                    postParam.newOneFiles = "2";
                } else if (this.addr.per == "SJ") {
                    postParam.newFiles = "2";
                }
            ...
            lists() { //返工详情
                if (this.type.typerukou == "add") {
                    ...
                    this.$set("tuyaList[(this.tuya.n-1)+this.tuya.index]", this.tuya)
                }
                this.$set("tuyaListss", this.tuyaList.slice(6 * (this.tuya.n - 1), 6 * this.tuya.n));
