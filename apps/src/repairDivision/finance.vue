	...
	<div class="row worksList clearfix">
		<div class="no-padding">
			<form class="form-horizontal col-sm-12">
				<div class="caiwu col-sm-12 control-label" style="min-width:1200px">
					<div>
						<label label class="control-label shijian">时间:</label>
					...
					<div>
						...
						<div class="col-sm-2">
							<select v-model="finance.operation" class="form-control shuruXZ">
								<option selected value="">全部</option>
								<option value="XT">修图</option>
								<option value="SJ">设计</option>
								<option value="TX">提现</option>
								<option value="CF">处罚</option>
								<option value="TD">退单</option>
								<option value="CD">撤单</option>
							</select>
						</div>
					</div>
				</div>
		...
		<div class="col-sm-12" style="min-height: 592px;background:white">
			<table class="table table-condensedn table-hover table-bordered">
				<thead>
					<tr>
						<th style="min-width:230px">订单号</th>
				...
				<tbody style="background:white">
					<tr v-for="findFinanceLog in findFinanceLog">
						...
						<td v-if="findFinanceLog.operation=='NONE'">无类型</td>
						<td>{{findFinanceLog.money}}</td>
						<td>{{findFinanceLog.type=="ADD"?"收入":"支出"}}</td>
					</tr>
				</tbody>
			</table>
			<p v-if="!findFinanceLog.length" style="font-size: 24px;margin-bottom: 100px;text-align: center;">
				暂无信息
			</p>
		</div>
...
export default {
	...
	methods: {
		daochu (){
			...
			var url = this.host + 'v1/finance/excel?workEmpId='+this.id+'&startTime='+(this.finance.time.split(" 至 ")[0])+'&endTime='+(this.finance.time.split(" 至 ")[1])+'&financeType='+this.finance.financeType
